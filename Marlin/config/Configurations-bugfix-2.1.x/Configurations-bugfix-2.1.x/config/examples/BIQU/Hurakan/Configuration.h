...
#define STRING_CONFIG_H_AUTHOR "(thisiskeithb, Hurakan)" // Original author or contributor.
 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 12.99 }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1549f }       // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.0605f }          // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.0876f } // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  #endif
 ...
 */
  ...
  //#define PRINTER_EVENT_LEDS // Disabled due to event LEDs using LCD backlight instead of hotend.
#endif
