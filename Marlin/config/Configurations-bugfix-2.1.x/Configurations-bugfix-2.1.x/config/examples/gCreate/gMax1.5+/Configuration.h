...
#define Z_CLEARANCE_BETWEEN_PROBES  6 // (mm) Z Clearance between probe points
#define Z_CLEARANCE_MULTI_PROBE     5 // (mm) Z Clearance between multiple probes
#define Z_PROBE_ERROR_TOLERANCE     3 // (mm) Tolerance for early trigger (<= -probe.offset.z + ZPET)
//#define Z_AFTER_PROBING           6 // (mm) Z position after probing is done
...
#define Z_SAFE_HOMING
  ...
  #define Z_SAFE_HOMING_X_POINT ((X_BED_SIZE) / 2 - 8) // (mm) X point for Z homing
  #define Z_SAFE_HOMING_Y_POINT ((Y_BED_SIZE) / 2 - 4) // (mm) Y point for Z homing
